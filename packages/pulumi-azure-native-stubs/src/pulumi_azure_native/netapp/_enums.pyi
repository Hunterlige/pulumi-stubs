

import builtins as _builtins
import pulumi
from enum import Enum

__all__ = ['ApplicationType', 'AvsDataStore', 'ChownMode', 'CifsChangeNotifyState', 'CoolAccessRetrievalPolicy', 'CoolAccessTieringPolicy', 'DayOfWeek', 'ElasticBackupPolicyState', 'ElasticNfsv3Access', 'ElasticNfsv4Access', 'ElasticPoolEncryptionKeySource', 'ElasticProtocolType', 'ElasticRootAccess', 'ElasticServiceLevel', 'ElasticSmbEncryption', 'ElasticUnixAccessRule', 'ElasticVolumePolicyEnforcement', 'EnableSubvolumes', 'EnableWriteBackState', 'EncryptionKeySource', 'EncryptionType', 'EndpointType', 'GlobalFileLockingState', 'KerberosState', 'KeySource', 'LdapServerType', 'LdapState', 'ManagedServiceIdentityType', 'NetworkFeatures', 'PolicyStatus', 'ProtocolTypes', 'QosType', 'ReplicationSchedule', 'SecurityStyle', 'ServiceLevel', 'SmbAccessBasedEnumeration', 'SmbEncryptionState', 'SmbNonBrowsable', 'SnapshotDirectoryVisibility', 'SnapshotUsage', 'Type']
@pulumi.type_token("azure-native:netapp:ApplicationType")
class ApplicationType(_builtins.str, Enum):
    
    SA_P_HANA = ...
    ORACLE = ...


@pulumi.type_token("azure-native:netapp:AvsDataStore")
class AvsDataStore(_builtins.str, Enum):
    
    ENABLED = ...
    DISABLED = ...


@pulumi.type_token("azure-native:netapp:ChownMode")
class ChownMode(_builtins.str, Enum):
    
    RESTRICTED = ...
    UNRESTRICTED = ...


@pulumi.type_token("azure-native:netapp:CifsChangeNotifyState")
class CifsChangeNotifyState(_builtins.str, Enum):
    
    DISABLED = ...
    ENABLED = ...


@pulumi.type_token("azure-native:netapp:CoolAccessRetrievalPolicy")
class CoolAccessRetrievalPolicy(_builtins.str, Enum):
    
    DEFAULT = ...
    ON_READ = ...
    NEVER = ...


@pulumi.type_token("azure-native:netapp:CoolAccessTieringPolicy")
class CoolAccessTieringPolicy(_builtins.str, Enum):
    
    AUTO = ...
    SNAPSHOT_ONLY = ...


@pulumi.type_token("azure-native:netapp:DayOfWeek")
class DayOfWeek(_builtins.str, Enum):
    
    SUNDAY = ...
    MONDAY = ...
    TUESDAY = ...
    WEDNESDAY = ...
    THURSDAY = ...
    FRIDAY = ...
    SATURDAY = ...


@pulumi.type_token("azure-native:netapp:ElasticBackupPolicyState")
class ElasticBackupPolicyState(_builtins.str, Enum):
    
    ENABLED = ...
    DISABLED = ...


@pulumi.type_token("azure-native:netapp:ElasticNfsv3Access")
class ElasticNfsv3Access(_builtins.str, Enum):
    
    ENABLED = ...
    DISABLED = ...


@pulumi.type_token("azure-native:netapp:ElasticNfsv4Access")
class ElasticNfsv4Access(_builtins.str, Enum):
    
    ENABLED = ...
    DISABLED = ...


@pulumi.type_token("azure-native:netapp:ElasticPoolEncryptionKeySource")
class ElasticPoolEncryptionKeySource(_builtins.str, Enum):
    
    NET_APP = ...
    KEY_VAULT = ...


@pulumi.type_token("azure-native:netapp:ElasticProtocolType")
class ElasticProtocolType(_builtins.str, Enum):
    
    NF_SV3 = ...
    NF_SV4 = ...
    SMB = ...


@pulumi.type_token("azure-native:netapp:ElasticRootAccess")
class ElasticRootAccess(_builtins.str, Enum):
    
    ENABLED = ...
    DISABLED = ...


@pulumi.type_token("azure-native:netapp:ElasticServiceLevel")
class ElasticServiceLevel(_builtins.str, Enum):
    
    ZONE_REDUNDANT = ...


@pulumi.type_token("azure-native:netapp:ElasticSmbEncryption")
class ElasticSmbEncryption(_builtins.str, Enum):
    
    ENABLED = ...
    DISABLED = ...


@pulumi.type_token("azure-native:netapp:ElasticUnixAccessRule")
class ElasticUnixAccessRule(_builtins.str, Enum):
    
    READ_ONLY = ...
    READ_WRITE = ...
    NO_ACCESS = ...


@pulumi.type_token("azure-native:netapp:ElasticVolumePolicyEnforcement")
class ElasticVolumePolicyEnforcement(_builtins.str, Enum):
    
    ENFORCED = ...
    NOT_ENFORCED = ...


@pulumi.type_token("azure-native:netapp:EnableSubvolumes")
class EnableSubvolumes(_builtins.str, Enum):
    
    ENABLED = ...
    DISABLED = ...


@pulumi.type_token("azure-native:netapp:EnableWriteBackState")
class EnableWriteBackState(_builtins.str, Enum):
    
    DISABLED = ...
    ENABLED = ...


@pulumi.type_token("azure-native:netapp:EncryptionKeySource")
class EncryptionKeySource(_builtins.str, Enum):
    
    MICROSOFT_NET_APP = ...
    MICROSOFT_KEY_VAULT = ...


@pulumi.type_token("azure-native:netapp:EncryptionType")
class EncryptionType(_builtins.str, Enum):
    
    SINGLE = ...
    DOUBLE = ...


@pulumi.type_token("azure-native:netapp:EndpointType")
class EndpointType(_builtins.str, Enum):
    
    SRC = ...
    DST = ...


@pulumi.type_token("azure-native:netapp:GlobalFileLockingState")
class GlobalFileLockingState(_builtins.str, Enum):
    
    DISABLED = ...
    ENABLED = ...


@pulumi.type_token("azure-native:netapp:KerberosState")
class KerberosState(_builtins.str, Enum):
    
    DISABLED = ...
    ENABLED = ...


@pulumi.type_token("azure-native:netapp:KeySource")
class KeySource(_builtins.str, Enum):
    
    MICROSOFT_NET_APP = ...
    MICROSOFT_KEY_VAULT = ...


@pulumi.type_token("azure-native:netapp:LdapServerType")
class LdapServerType(_builtins.str, Enum):
    
    ACTIVE_DIRECTORY = ...
    OPEN_LDAP = ...


@pulumi.type_token("azure-native:netapp:LdapState")
class LdapState(_builtins.str, Enum):
    
    DISABLED = ...
    ENABLED = ...


@pulumi.type_token("azure-native:netapp:ManagedServiceIdentityType")
class ManagedServiceIdentityType(_builtins.str, Enum):
    
    NONE = ...
    SYSTEM_ASSIGNED = ...
    USER_ASSIGNED = ...
    SYSTEM_ASSIGNED_USER_ASSIGNED = ...


@pulumi.type_token("azure-native:netapp:NetworkFeatures")
class NetworkFeatures(_builtins.str, Enum):
    
    BASIC = ...
    STANDARD = ...
    BASIC_STANDARD = ...
    STANDARD_BASIC = ...


@pulumi.type_token("azure-native:netapp:PolicyStatus")
class PolicyStatus(_builtins.str, Enum):
    
    ENABLED = ...
    DISABLED = ...


@pulumi.type_token("azure-native:netapp:ProtocolTypes")
class ProtocolTypes(_builtins.str, Enum):
    
    NF_SV3 = ...
    NF_SV4 = ...
    SMB = ...


@pulumi.type_token("azure-native:netapp:QosType")
class QosType(_builtins.str, Enum):
    
    AUTO = ...
    MANUAL = ...


@pulumi.type_token("azure-native:netapp:ReplicationSchedule")
class ReplicationSchedule(_builtins.str, Enum):
    
    REPLICATION_SCHEDULE_10MINUTELY = ...
    HOURLY = ...
    DAILY = ...


@pulumi.type_token("azure-native:netapp:SecurityStyle")
class SecurityStyle(_builtins.str, Enum):
    
    NTFS = ...
    UNIX = ...


@pulumi.type_token("azure-native:netapp:ServiceLevel")
class ServiceLevel(_builtins.str, Enum):
    
    STANDARD = ...
    PREMIUM = ...
    ULTRA = ...
    STANDARD_ZRS = ...


@pulumi.type_token("azure-native:netapp:SmbAccessBasedEnumeration")
class SmbAccessBasedEnumeration(_builtins.str, Enum):
    
    DISABLED = ...
    ENABLED = ...


@pulumi.type_token("azure-native:netapp:SmbEncryptionState")
class SmbEncryptionState(_builtins.str, Enum):
    
    DISABLED = ...
    ENABLED = ...


@pulumi.type_token("azure-native:netapp:SmbNonBrowsable")
class SmbNonBrowsable(_builtins.str, Enum):
    
    DISABLED = ...
    ENABLED = ...


@pulumi.type_token("azure-native:netapp:SnapshotDirectoryVisibility")
class SnapshotDirectoryVisibility(_builtins.str, Enum):
    
    HIDDEN = ...
    VISIBLE = ...


@pulumi.type_token("azure-native:netapp:SnapshotUsage")
class SnapshotUsage(_builtins.str, Enum):
    
    USE_EXISTING_SNAPSHOT = ...
    CREATE_NEW_SNAPSHOT = ...


@pulumi.type_token("azure-native:netapp:Type")
class Type(_builtins.str, Enum):
    
    DEFAULT_USER_QUOTA = ...
    DEFAULT_GROUP_QUOTA = ...
    INDIVIDUAL_USER_QUOTA = ...
    INDIVIDUAL_GROUP_QUOTA = ...


