

import builtins as _builtins
import pulumi
from enum import Enum

__all__ = ['Action', 'EncryptionType', 'IdentityType', 'PrivateEndpointServiceConnectionStatus', 'PublicNetworkAccess', 'SkuName', 'SkuTier', 'StorageTargetType', 'VolumeCreateOption']
@pulumi.type_token("azure-native:elasticsan:Action")
class Action(_builtins.str, Enum):
    
    ALLOW = ...


@pulumi.type_token("azure-native:elasticsan:EncryptionType")
class EncryptionType(_builtins.str, Enum):
    
    ENCRYPTION_AT_REST_WITH_PLATFORM_KEY = ...
    ENCRYPTION_AT_REST_WITH_CUSTOMER_MANAGED_KEY = ...


@pulumi.type_token("azure-native:elasticsan:IdentityType")
class IdentityType(_builtins.str, Enum):
    
    NONE = ...
    SYSTEM_ASSIGNED = ...
    USER_ASSIGNED = ...


@pulumi.type_token(...)
class PrivateEndpointServiceConnectionStatus(_builtins.str, Enum):
    
    PENDING = ...
    APPROVED = ...
    FAILED = ...
    REJECTED = ...


@pulumi.type_token("azure-native:elasticsan:PublicNetworkAccess")
class PublicNetworkAccess(_builtins.str, Enum):
    
    ENABLED = ...
    DISABLED = ...


@pulumi.type_token("azure-native:elasticsan:SkuName")
class SkuName(_builtins.str, Enum):
    
    PREMIUM_LRS = ...
    PREMIUM_ZRS = ...


@pulumi.type_token("azure-native:elasticsan:SkuTier")
class SkuTier(_builtins.str, Enum):
    
    PREMIUM = ...


@pulumi.type_token("azure-native:elasticsan:StorageTargetType")
class StorageTargetType(_builtins.str, Enum):
    
    ISCSI = ...
    NONE = ...


@pulumi.type_token("azure-native:elasticsan:VolumeCreateOption")
class VolumeCreateOption(_builtins.str, Enum):
    
    NONE = ...
    VOLUME_SNAPSHOT = ...
    DISK_SNAPSHOT = ...
    DISK = ...
    DISK_RESTORE_POINT = ...


